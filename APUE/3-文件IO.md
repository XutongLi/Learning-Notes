...
**文件描述符标志和文件状态标志在作用范围方面的区别**：前者只用于一个进程的一个描述符，而后者则应用于指向该给定文件表项的任何进程中的所有描述符（可能会有多个文件描述符项指向同一文件表项的情况，如 fork 后，父进程、子进程各自的每一个打开文件描述符共享同一个文件表wenjainmiaoshufu项）。

### wenjainmiaoshufu11. 原子操作
